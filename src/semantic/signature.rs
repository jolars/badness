...
// `.sty`/`.cls` stem, generated by `scripts/gen_package_names.py` from the same
// pinned tlpdb as the name lists (see `data/package_metadata.json`). A *shipped,
// static* dataset — the descriptions the TEXMF file scan cannot cheaply derive —
// consumed by package hover and completion detail, never a runtime distro query.
const PACKAGE_METADATA_JSON: &str = include_str!("../../data/package_metadata.json");

/// CTAN metadata for one package/class stem: an optional one-line description and
/// the CTAN catalogue id (for a `https://ctan.org/pkg/<id>` URL).
...
impl PackageMeta {
    ...
    pub fn ctan_url(&self) -> Option<String> {
        self.ctan
            .as_deref()
            .map(|id| format!("https://ctan.org/pkg/{id}"))
    }
}

/// The on-disk `package_metadata.json` shape: a `note` header (ignored) plus the
/// `stem -> PackageMeta` map.
#[derive(Deserialize)]
struct MetadataFile {
    entries: HashMap<String, PackageMeta>,
}

static PACKAGE_METADATA: LazyLock<HashMap<String, PackageMeta>> = LazyLock::new(|| {
    serde_json::from_str::<MetadataFile>(PACKAGE_METADATA_JSON)
        .expect("baked package_metadata.json is valid")
        .entries
});
...
/// the owning package's description + CTAN id. See [`PACKAGE_METADATA_JSON`].
...
mod tests {
    ...
    fn package_metadata_resolves_stem_to_ctan_facts() {
        ...
        assert_eq!(
            meta.desc.as_deref(),
            Some("AMS mathematical facilities for LaTeX")
        );
        assert_eq!(
