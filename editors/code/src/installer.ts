...
function isCandidateBadnessCliReleaseTag(tag: string): boolean {
...
function detectTargetAsset(): TargetAsset {
  ...
  if (process.platform === "darwin" && process.arch === "arm64") {
    return { archiveNames: ["badness-aarch64-apple-darwin.tar.gz"], binaryName };
  }
  if (process.platform === "darwin" && process.arch === "x64") {
    return { archiveNames: ["badness-x86_64-apple-darwin.tar.gz"], binaryName };
  }
  if (process.platform === "linux" && process.arch === "arm64") {
    return {
      archiveNames: [
        "badness-aarch64-unknown-linux-gnu.tar.gz",
        "badness-aarch64-unknown-linux-musl.tar.gz",
      ],
      binaryName,
    };
  }
  if (process.platform === "linux" && process.arch === "x64") {
    return {
      archiveNames: [
        "badness-x86_64-unknown-linux-gnu.tar.gz",
        "badness-x86_64-unknown-linux-musl.tar.gz",
      ],
      binaryName,
    };
  }
  if (process.platform === "win32" && process.arch === "x64") {
    return { archiveNames: ["badness-x86_64-pc-windows-msvc.zip"], binaryName };
  }
  if (process.platform === "win32" && process.arch === "arm64") {
    return { archiveNames: ["badness-aarch64-pc-windows-msvc.zip"], binaryName };
  }
...
export async function resolveBadnessBinary(
        ...
        async () => {
          ...
          if (candidate.kind === "latest-list") {
            ...
            for (const release of releases) {
              ...
              if (!isCandidateBadnessCliReleaseTag(release.tag_name)) {
                continue;
              }
            ...
            throw new Error(
              `No stable Badness CLI release asset '${target.archiveNames.join("' or '")}' found for ${repo}@latest`,
            );
          }
          ...
          if (
            candidate.kind === "latest-pointer" &&
            !isCandidateBadnessCliReleaseTag(release.tag_name)
          ) {
            throw new Error(
              `${repo}/releases/latest points at '${release.tag_name}', which is not a Badness CLI release`,
            );
          }
