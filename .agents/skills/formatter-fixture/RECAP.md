...
- Re-survey `environments`, `mand-args`, and `opt-args` for comments glued to an
  opening or closing delimiter (`{%` or `]%`) in a declared environment header.
...
| Declared environment arguments | `environment_argument_comment_barrier`, `environment_argument_comment_slots`, `environment_argument_escaped_delimiters`, `environment_omitted_optional_slots` |
