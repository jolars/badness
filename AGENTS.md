...
It is a **three-crate Cargo workspace** (edition 2024) whose root package is the
...
- **`badness-formatter`** — the layout engine (`core`, `ir`, `printer`, `style`,
  ...
  keep building for `wasm32-unknown-unknown` — a CI job guards this. Anything
  touching the filesystem, threads, or processes stays in the root crate.
