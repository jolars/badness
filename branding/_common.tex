...
% --- Logo geometry (all in cm) ----------------------------------------------
%
% The mark's SHAPE is canonical and lives here — artifacts must not override
% the shape knobs, only \bnMarkScale, so the logo is identical everywhere.

\providecommand{\bnBarCount}{4}     % number of horizontal bars
\providecommand{\bnBarWidth}{3.6}   % length of an in-margin bar
...
\providecommand{\bnJutExtra}{1.05}  % how far it sticks out
\providecommand{\bnRound}{0.07}     % corner rounding
\providecommand{\bnShowMargin}{0}   % 1 = draw the dashed margin guide, 0 = hide
\providecommand{\bnMarkScale}{1}    % uniform scale factor for the whole mark
...
% Everything (including corner rounding) is multiplied by \bnMarkScale.

\newcommand{\bnDrawMark}{%
  \pgfmathtruncatemacro{\bnLast}{\bnBarCount-1}%
  \pgfmathsetmacro{\bnW}{\bnBarWidth*\bnMarkScale}%
  \pgfmathsetmacro{\bnH}{\bnBarHeight*\bnMarkScale}%
  \pgfmathsetmacro{\bnG}{\bnBarGap*\bnMarkScale}%
  \pgfmathsetmacro{\bnJut}{\bnJutExtra*\bnMarkScale}%
  \pgfmathsetmacro{\bnR}{\bnRound*\bnMarkScale}%
  \pgfmathsetmacro{\bnTotalH}{\bnBarCount*\bnH + \bnLast*\bnG}%
  \ifnum\bnShowMargin=1
    ...
    (\bnW, -0.12*\bnMarkScale) -- (\bnW, \bnTotalH+0.12*\bnMarkScale);
  \fi
  \foreach \i in {0,...,\bnLast}{%
      \pgfmathsetmacro{\bny}{(\bnLast-\i)*(\bnH+\bnG)}%
      \pgfmathtruncatemacro{\bnNum}{\i+1}%
      \ifnum\bnNum=\bnJutWhich
        \fill[bnBad, rounded corners=\bnR cm]
        (0,\bny) rectangle (\bnW+\bnJut, \bny+\bnH);
      \else
        \fill[bnInk, rounded corners=\bnR cm]
        (0,\bny) rectangle (\bnW, \bny+\bnH);
      \fi
    }%
}
