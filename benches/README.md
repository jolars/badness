...
Six complementary tools, measuring different things:
  ...
  | `benches/compare_lsp_memory.sh` (`task bench:lsp`) | LSP latency and process-tree RSS/PSS vs TexLab    | **yes** (whole process tree)  |
...
The two LSP memory harnesses answer different questions. The external comparison
measures warm request latency and the operating-system cost of ordinary, fresh
Badness and TexLab editor sessions—whole-process-tree RSS and PSS—over a
complete pinned thesis workspace. The in-process harness asks whether Badness
retains *history* after arriving at the same current project state. Its counting
allocator excludes free pages the allocator has not returned to the operating
system.
...
Python driver performs initialization and diagnostics, then primes document
symbols and citation/reference hovers. Once the session settles, it measures
warm document symbols, hover, go to definition, find references, and rename. The
three navigation operations use the pinned `Aup91` citation, whose BibTeX entry
lives in another file. Rename constructs but does not apply its edit.

Every request target gets two warmups and 20 measured rounds per fresh session.
The artifact aggregates median and p95 latency across the sessions and records
the symbols, locations, edits, files, and serialized payload returned. It also
samples every descendant process from Linux `/proc` at 150 ms intervals.
Baseline and settled readings require five seconds below 5% of one CPU core; a
phase that has not settled after 60 seconds fails the run. The committed
artifact reports median readiness and memory milestones while retaining each raw
run's summary.
...
SETTLE_TIMEOUT=120 task bench:lsp

# Shorten request sampling while debugging the harness.
LSP_LATENCY_RUNS=2 LSP_LATENCY_WARMUPS=0 task bench:lsp
```

This is an external editor-session comparison, not a feature-equivalence claim.
TexLab and Badness analyze different things, and both latency and RSS are
machine-dependent. The complete workspace is intentional: even though only five
buffers are open, each server may discover supporting `.cls`, `.sty`, and `.bib`
files.
