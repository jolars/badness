% The mandatory-group mirror of `keyval_optionals.tex`: every command or
% environment here has a curated `{"kind": "req", "content": "keyval"}` slot,
% so the formatter may split its brace argument at a comma the author *glued* —
% materializing a space token TeX sees. Each key list is authored on one long
% line so the split actually fires.
...
x = 1
\end{lstlisting}
\begin{tblr}{colspec={ll},hlines,vlines,row{1}={font=\bfseries},cell{2}{1}={bg=yellow!20},}
  Alpha & Beta \\
  Gamma & Delta \\
\end{tblr}
\end{document}
