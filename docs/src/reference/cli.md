...
Format LaTeX source. With paths, formats each file in place. With no paths,
reads standard input and writes the formatted result to standard output.
...
Lint LaTeX source, reporting parse diagnostics. With paths, lints each file.
With no paths, reads standard input. Exits non-zero if any diagnostics are
reported.
...
Run the language server over stdio. See [Editor
Setup](../guide/editor-setup.md).
