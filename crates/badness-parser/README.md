...
scanning, expl3 support, labels, and document outlines. Meaning never leaks
into the grammar.

The crate builds for `wasm32-unknown-unknown`.

Entry points: `parser::parse` / `parser::parse_with_flavor`,
`bib::parse`, `semantic::SemanticModel::build`, and the typed wrappers in
`ast`.
