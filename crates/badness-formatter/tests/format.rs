...
const FIXTURES: &[(&str, WrapMode, usize)] = &[
    ...
    // head. Likewise, comment-terminated command-only lines before the eventual
    // TikZ statement are prefixes, not continuations. They and the head stay at
    // body indentation, while width wraps and tails after genuine mid-statement
    // comments still hang (issue #158).
