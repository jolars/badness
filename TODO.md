...
- [x] **Package/class name completion** for `\usepackage{}` / `\documentclass{}`
  (`package_arg` in `src/completion.rs`, `PackageName` context). Sourced from a
  baked `.sty`/`.cls` name list (`data/{package,class}_names.txt`, generated by
  `scripts/gen_package_names.py` from the pinned TeX Live tlpdb; names only, ranked
  namesake/common-first) merged with locally discovered `.sty`/`.cls` files in the
  LSP layer (`package_completion_items`). The kpsewhich/distro DB stays out of
  scope (same texmf/CTAN boundary as CWL ingest and the include graph); a runtime
  texmf-discovery tier is designed but deferred (opt-in; would cross that boundary).
